# Integrating TradingCalendar

## From Solidity

```solidity
import {TradingCalendarHook} from "trading-calendar/src/hooks/TradingCalendarHook.sol";
...
hook.configure(key, TradingCalendarHook.Config({ /* see the README */ }));
...
const address = hookAddress("trading-calendar", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "TradingCalendar"
