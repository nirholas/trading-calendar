...
contract DeployLocal is Script {
    ...
    function run() external {
        ...
        (address predicted, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER, FLAGS, type(TradingCalendarHook).creationCode, abi.encode(IPoolManager(address(manager)))
        );
        hook = new TradingCalendarHook{salt: salt}(IPoolManager(address(manager)));
    ...
    function _openPool() private {
        ...
        hook.configure(key, TradingCalendarHook.Config({openSecond: 13 hours + 30 minutes, closeSecond: 20 hours, rampSeconds: 30 minutes, sessionFee: 500, closedFee: 20_000, daysMask: 0x1F}));
