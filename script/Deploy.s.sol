...
/**
 * @title DeployTradingCalendar
 * @notice Deploys TradingCalendar deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployTradingCalendar is Script {
    ...
    function run() external {
        ...
        bytes memory creationCode = type(TradingCalendarHook).creationCode;
        ...
        console2.log("TradingCalendar ", predicted);
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("TradingCalendar")),
